...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        long sumDays = project.getTaskLists().stream()
                ...
                .map(Task::getCreated)
                .mapToLong(date -> date.until(LocalDate.now()).getDays()).sum();
        ...
        double averageWorkingOnTask = sumDays / countTasks;
        System.out.println("average: " + averageWorkingOnTask);

//        project.getTaskLists().stream()
//                .filter(inProgressTasks::contains)
//                .flatMap(tl -> tl.getTasks().stream())
//                .map(Task::getCreated)
//                .map(date -> date.until(LocalDate.now()).getDays())
//                .forEach(System.out::println);

        double averageWorkingOnTask2 = project.getTaskLists().stream()
                        .filter(inProgressTasks::contains)
                        .flatMap(tl -> tl.getTasks().stream())
                        .map(Task::getCreated)
                        .mapToInt(date -> date.until(LocalDate.now()).getDays())
                        .average().getAsDouble();
